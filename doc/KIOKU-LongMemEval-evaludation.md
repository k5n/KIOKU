...
LongMemEval 実行は `[prompt.longmemeval_kioku]` と `[judge]` を前提にします。
...
LongMemEval 実行時の prompt 設定は `longmemeval_kioku` に一本化されています。
...
LongMemEval 実行では `prompt_context` は必須です。  
backend が `prompt_context = None` を返した場合、runner は fail-fast します。
