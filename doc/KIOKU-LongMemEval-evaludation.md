...
つまり、この仕様で judge が見る retrieval 入力は次です。

- `QueryOutput.retrieved`
  - backend が返した構造化 retrieval item
- `QueryOutput.prompt_context`
  - Answerer に渡すために整形済みのコンテキスト

このうち retrieval judge が使うのは **`prompt_context.text`** です。
...
KIOKU backend は LongMemEval 実行時に `QueryOutput.prompt_context` を **必ず**返すものとします。

v1 の仕様では、LongMemEval + KIOKU に対して `prompt_context = None` は不正です。  
fallback で raw retrieved items を render する provisional 動作は使いません。
...
for case in longmemeval_cases {
    ...
    let context = query_output
        .prompt_context
        .expect("longmemeval_kioku_v1 requires prompt_context");
...
current `crates/evaluate` には `[prompt.longmemeval]` がありますが、これは provisional / official-ish answer profile 向けです。  
`longmemeval_kioku_v1` では、LoCoMo と同様に KIOKU 専用 prompt 設定を別で持つ方が安全です。
...
## 20. current codebase に対して必要な変更

`longmemeval_kioku_v1` を実装するには、少なくとも次の変更が必要です。

### 20.1 provisional LongMemEval path と protocol-specific path を分ける

current `LongMemEvalJudge` は exact match に近い暫定実装で、official の type-specific semantics を表せていません。  
したがって、`longmemeval_kioku_v1` は current path を置き換えるのではなく、**別 protocol / 別 runner** として追加する方が安全です。

### 20.2 Judge を 2 系統にする

LoCoMo と同様に次の 2 つが必要です。

- `RetrievalJudge`
- `AnswerJudge`

LongMemEval ではこの 2 つの prompt が question type ごとに rubric を切り替える必要があります。

### 20.3 LongMemEval 実行時に `prompt_context` を必須化する

KIOKU backend で `prompt_context = None` を許すと、retrieval judge が評価すべき対象が曖昧になります。  
そのため LongMemEval + KIOKU 実行では `prompt_context` を必須にします。

### 20.4 retrieval log schema を拡張する

current `RetrievalLogRecord` には `question_type` がありません。  
しかし `longmemeval_kioku_v1` の `retrieval.jsonl` では question type ごとの分析を行うため、
`question_type` を log に保持する必要があります。

また、abstention question では `is_sufficient` / `score` / `label` を `null` にできるよう、
serialization policy もこの仕様に合わせる必要があります。

### 20.5 metrics report schema を拡張する

current `MetricsReport` / `DatasetMetrics` には、少なくとも次がありません。
...
LongMemEval v1 では per-type macro 指標と abstention 系カウントを main report に載せる必要があります。

また、`prompt_context.text` を tokenize して得た context サイズを比較可能にするため、
少なくとも次の provenance も保持する必要があります。

- `context_tokenizer`
- `context_token_count_scope`

加えて、current `MetricsReport` では provenance が top-level flatten されていますが、  
この仕様の `metrics.json` では `provenance` object をネストして持つ前提です。  
したがって implementation をこの schema に寄せるか、逆に仕様側を flat schema に合わせるかを決める必要があります。

### 20.6 retrieval log に context token 数を追加する

`longmemeval_kioku_v1` では retrieval の評価対象が `prompt_context.text` なので、  
question ごとの artifact にもその大きさを残す必要があります。

少なくとも `retrieval.jsonl` に `context_token_count` を追加する必要があります。

### 20.7 config schema を拡張する

current `PromptConfig` は `longmemeval` と `locomo_kioku` しか持っていません。  
`longmemeval_kioku` 用の prompt section を追加する必要があります。
