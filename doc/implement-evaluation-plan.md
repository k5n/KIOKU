...
- `evidence` の `dia_id` を `evidence_event_ids` に入れる
...
- turn に `has_answer` があれば `evidence_event_ids` にも落とす
...
4. `LlmAnswerer` を作る
5. `LoCoMoJudge` と `LongMemEvalJudge` を作る
6. overall / per-category / per-type accuracy を出す

この段階で「全履歴を返すだけでベンチマークが回る」状態にします。

### Phase 2: retrieval 指標を入れる
