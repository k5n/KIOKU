...
6. `return-all` の `max_items` は「時系列で新しい N 件を選び、返却順は時系列昇順」として固定する
7. `max_tokens` は Phase 1 では未対応とし、CLI で指定された場合はエラーにする
8. まず full-context ベースラインを作り、その後に本物の記憶検索へ差し替える
