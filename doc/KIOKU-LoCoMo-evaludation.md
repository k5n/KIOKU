...
KIOKU backend は LoCoMo 実行時に `QueryOutput.prompt_context` を **必ず**返すものとします。

v1 の仕様では、LoCoMo + KIOKU に対して `prompt_context = None` は不正です。  
fallback で raw retrieved items を render する official 互換動作は使いません。
...
for case in locomo_cases {
    ...
    for question in case.questions.filter(category in 1..=4) {
        ...
        let context = query_output.prompt_context.expect("locomo_kioku_v1 requires prompt_context");
...
`average_retrieved_item_count` は、KIOKU backend が 1 question あたり平均何件の retrieval item を返したかです。  
これは prompt size の proxy として使います。
...
```json
{
  ...
  "retrieved_source_event_ids": [
    "locomo:sample:0:event:D3:4",
    "locomo:sample:0:event:D5:2"
  ],
...
current `RetrievalLogRecord` は `retrieved_event_ids` を前提にしています。  
KIOKU では retrieval item が event とは限らないため、v1 では次の変更が必要です。

- `retrieved_event_ids` を `retrieved_memory_ids` へ改名する
- `retrieved_source_event_ids` を追加する
- `context_kind` を追加する
- `context_text` を追加する
- retrieval judge の結果を追加する
...
```json
{
  ...
  "metrics": {
    ...
    "average_retrieved_item_count": 8.3,
...
そのため LoCoMo + KIOKU 実行では `prompt_context` を必須にします。

### 18.4 retrieval log schema を一般化する

event-centric な `retrieved_event_ids` では KIOKU の facts / relations を表せません。  
schema を memory-centric に変更する必要があります。
