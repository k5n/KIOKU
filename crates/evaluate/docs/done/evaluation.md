...
- `session_X` と `session_X_date_time` の動的キーをパースし、各 session に開始日時とメッセージ列が 1:1 で存在することを検証する
- session は `session_1`, `session_2`, ... の数値順で処理する
...
Phase 1 のように暫定 judge を使う段階では、同じ `accuracy` という語でも最終 benchmark metric とは意味論が異なるため、`metrics.json` や `answers.jsonl` に `judge_kind`、`metric_semantics_version`、`provisional` を持たせて区別する方が安全です。
...
したがって LoCoMo の `overall accuracy` は、`kioku` では **`category 1-4` のみを分母にした main score** を指すものとして固定し、`category 5` は含めません。
...
このとき adapter ごとに ID 形式がぶれないよう、LoCoMo では `question_id = locomo:{sample_id}:q{idx}`、`event_id = locomo:{sample_id}:event:{dia_id}` のような canonical ID を先に固定し、`evidence_event_ids` や retrieval log でも同じ値を使うべきです。
...
4. 全 session を入れ終わった後に `query` を呼ぶ

ここでの `question_date` は、検索対象をその時刻で打ち切るためのものではありません。  
`question_date` は「質問がどの時点の世界状態を問うているか」を表す参照時刻であり、backend や answerer が時間表現を解釈できるように query input に含めます。

公式 README では、`longmemeval_s_cleaned.json` と `longmemeval_m_cleaned.json` の `haystack_session_ids` は timestamp 順に並んでいると明記されています。  
ただし `longmemeval_oracle.json` は必ずしも sort 済みではないので、runner 側で日付順に sort しておくと安全です。加えて、`haystack_dates` / `haystack_session_ids` / `haystack_sessions` は parallel array なので、loader で長さ一致を fail-fast で検証してから扱うべきです。

turn-level retrieval を安定して集計するため、LongMemEval 側も `question_id = longmemeval:{raw_question_id}`、`event_id = longmemeval:{raw_question_id}:{session_id}:t{turn_idx}` のような canonical ID 規則を固定し、`has_answer == true` の turn から作る `evidence_event_ids` も同じ規則に揃えるべきです。

### 4.3 LongMemEval の正式な answer 評価

LongMemEval の QA 評価は、LoCoMo のような単一 judge prompt ではなく、**question type ごとに rubric が異なる** のが重要です。  
これは少なくとも公式 `evaluate_qa.py` で明示されている方針です。
...
ただし、`kioku` の最終方針としては「official judge をそのまま埋め込む」のではなく、  
**official の task semantics は残しつつ、KIOKU 用 protocol で answer judge / retrieval judge を再定義する** 方がよいです。
...
KIOKU 用 protocol でも、retrieval headline metrics の分母から abstention を外すのが自然です。
...
### 4.7 KIOKU 用 LongMemEval protocol の要点

KIOKU 用 LongMemEval では、official retrieval metrics を headline にするより、  
**KIOKU が実際に Answerer へ渡した `prompt_context.text` が十分だったか** を測る方が解釈しやすいです。

そのため、LongMemEval でも LoCoMo と同様に次の protocol を採るのがよいです。

- answer correctness は LLM judge で採点する
- retrieval sufficiency も LLM judge で採点する
- retrieval judge と answerer は同じ `prompt_context.text` を見る
- answer judge / retrieval judge は official の question-type semantics を rubric に残す
- answer headline は `overall`, `task-averaged`, `abstention`, `per-type`
- retrieval headline は `overall`, `task-averaged`, `per-type`
- retrieval headline の分母から abstention を除外する

この protocol の詳細は [KIOKU-LongMemEval-evaludation.md](/home/nakatani/Projects/k5n/kioku/doc/KIOKU-LongMemEval-evaludation.md) を参照します。
...
4. LongMemEval も official leaderboard 再現ではなく `longmemeval_kioku_v1` として別 protocol 化する
5. LongMemEval の judge には official の type-specific semantics を残す
6. retrieval 指標と answer 指標を必ず分離して保存する
7. `return-all` の `max_items` は「時系列で新しい N 件を選び、返却順は時系列昇順」として固定する
8. `max_tokens` は Phase 1 では未対応とし、CLI で指定された場合はエラーにする
9. まず full-context ベースラインを作り、その後に本物の記憶検索へ差し替える
